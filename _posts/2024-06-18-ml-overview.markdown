...
主要的KKT条件便是 (2) 和 (5) ，只要满足这两个条件便可直接用拉格朗日乘子法。
...
宏查准率(Macro Precision) = (1 / n) * (类别1的查准率 + 类别2的查准率 + ... + 类别n的查准率)
...
- 交叉验证 t 检验：k折交叉验证；5*2交叉验证
- McNemar检验（基于列联表，卡方检验）

多学习器比较

- Friedman 检验 （基于序值，F检验；判断“是否都相同”），Friedman检验图
- Nemenyi 后续检验 （基于序值，进一步判断两两差别）

**偏差-方差分解**

对回归任务，泛化误差可通过“偏差-方差分解”拆解为 $E(f;D) = bias^2(\boldsymbol{x}) + var(\boldsymbol{x}) + \varepsilon^2$

偏差-方差窘境：训练不足时，学习器拟合能力不强，偏差主导；训练充足后，学习器的拟合能力很强，方差主导
...
若从二分上推广，有 OvO、OvR、MvM 几种方法，最常用的 MvM 技术是纠错输出码 (Error Correcting Output Codes, **ECOC**) 编码越长，纠错能力越强
...
在西瓜书上，两者的考察标准都是在验证集上的精度(accuracy)。
...
单变量决策树在每个非叶结点仅考虑一个划分属性，产生“轴平行”分类面

多变量决策树的每个非叶结点不仅考虑一个属性，例如“斜决策树”不是为每个非叶结点寻找最优划分属性，而是建立一个线性分类器
...
将高维数据映射到低维空间，高维空间中相似的样本点映射到网络输出层中邻近神经元，每个神经元拥有一个权向量。目标：为每个输出层神经元找到合适的权向量以保持拓扑结构

**CC：Cascade-Corrrelation** （级联相关）

构造性神经网络：将网络的结构也当作学习的目标之一，希望在训练过程中找到适合数据的网络结构

**Elman网络**

递归神经网络，Recurrent NN，Recursive NN

网络中可以有环形结构，可让一些神经元的输出反馈回来作为输入

**深度学习**

常用tricks

- 预训练+微调 （将大量参数分组，对每组先找到较好的局部配置，再全局寻优）
- 权共享 （减少需优化的参数）
- DropOut （降低 Rademacher 复杂度）
- ReLU（求导容易，缓解梯度消失）
- 交叉熵（代替均方误差，更能体现分类任务的特性）
...
SPODE 假设所有属性都依赖于同一属性，称为“超父”(Super-Parent)，然后通过交叉验证等模型选择方法确定超父属性

AODE 更上一层，尝试将每个属性作为超父构建 SPODE
...
多样性度量非常多，有不合度量、相关系数、Q-统计量、$\kappa$-统计量等。

多样性增强常用策略：数据样本扰动、输入属性扰动、输出表示扰动、算法参数扰动......