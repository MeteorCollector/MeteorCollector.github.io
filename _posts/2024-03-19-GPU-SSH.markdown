...
注意，当终端退出时，挂在 `2000` 端口的 `Carla` 不会停止。停止它的一种方法是在原终端直接 `Ctrl + C`，另一种是直接杀掉进程。（`ps aux | grep Carla` 获得进程号，然后 `kill -9 <pid>`）；

`datagen` 脚本比较烦人，对它进行 `Ctrl + C` 仅仅会停止当前组数据的采集，它还会开始下一组。直接关掉终端或者杀掉进程会比较管用。
